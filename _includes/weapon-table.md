...
{%- for weapon in sorted-weapons-category -%}
  ...
  {% unless requirement == weapon.REQUIREMENT_TYPE -%}
    {% continue %}
  {%- endunless %}
  |{::nomarkdown}<span class="{{ weapon.ELEMENT | downcase }}"><span class="record-name">{{ weapon.NAME }}</span></span>{:/nomarkdown}{%- if include.type == "Warrior" -%}<br /><span class="bar-descriptor">{{ weapon.CATEGORY }}</span>{%- endif -%}<br />![{{ weapon.NAME }}](/assets/img/weapons/{{ weapon.NAME }}.gif)|{{ weapon.REQUIREMENT_VALUE }}|
